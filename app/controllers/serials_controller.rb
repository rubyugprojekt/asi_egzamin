class SerialsController < ApplicationController
    ...
    before_action :authenticate_user!, only:[:new, :edit]
       ...
       def index
        ...
        @serials = Serial.all.order("created_at DESC").paginate(:page => params[:page], :per_page => 2)
        else
        @category_id = Category.find_by(nazwa: params[:category]).id
        @serials = Serial.where(:category_id => @category_id).order("created_at DESC").paginate(:page => params[:page], :per_page => 2)
        end
